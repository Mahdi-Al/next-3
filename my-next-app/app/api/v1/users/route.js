...
export async function DELETE(req) {
  ...
  const index = userData.findIndex((post) => post.id === parseInt(id));
...
export async function PATCH(req) {
  const { searchParams } = new URL(req.url);
  const id = searchParams.get("id");
  const userIndex = userData.findIndex((post) => post.id === parseInt(id));

  if (userIndex > -1) {
    const updatedPost = await req.json();
    userData[userIndex] = { ...userData[userIndex], ...updatedPost };
...
export async function GETbyId(req) {
  ...
  if (index > -1) {
    return new Response(userData[index], { status: 204 });
  } else {
    return new Response(JSON.stringify({ message: "Post not found" }), {
      status: 404,
      headers: { "Content-Type": "application/json" },
